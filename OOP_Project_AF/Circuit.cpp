...
#include <map>       // For map
#include <cmath>     // For log10, pow, fmod, abs
#include <iomanip>   // For setprecision, setw
#include <regex>     // For parsing V(...) and I(...) strings
...
void Circuit::deleteElement(const string& name) {
    ...
    auto it=remove_if(elements.begin(),elements.end(),
        [&upperName](const unique_ptr<Element>& el) {
...
void Circuit::buildACMNAMatrix(Matrix<Complex>& A_ac,vector<Complex>& b_ac,double frequency) {
    ...
    // cout << "\n--- Building AC MNA Matrix (Frequency: " << frequency << " Hz) ---\n";
    // cout << "Number of non-GND nodes: " << numNonGNDNodes << endl;
    // cout << "Number of elements adding branch currents: " << voltageSourceNameToCurrentIndex.size() << endl;
    // cout << "Total MNA matrix size: " << mnaSize << "x" << mnaSize << endl;
    ...
    for(const auto& element:elements) {
        ...
        if(type=="RESISTOR"||type=="CAPACITOR") { // Inductors handled separately below
            if(idx1!=-1) {
    ...
    // cout << "--- AC MNA Matrix Building Complete ---\n";
}
            ...
            for(const auto& el:elements) {
                ...
                if(type=="VOLTAGESOURCE"||type=="INDUCTOR") {
                    ...
                    if(it!=voltageSourceNameToCurrentIndex.end()) {
                        ...
                        // This case should ideally not happen if updateNodesAndBranchCurrentIndices is correct
...
void Circuit::solveTransient(double tstep,double tstop,double tstart,double tmaxstep) {
    ...
    while(currentTime<=tstop+1e-9) { // Add a small epsilon to ensure tstop is included
        ...
        // --- DEBUGGING PRINTS ---
        cout<<"\n--- DEBUG: MNA Matrix A at time "<<currentTime<<"s ---\n";
        A.print("DEBUG: MNA Matrix A"); // Use the print method with title
        cout<<"--- DEBUG: RHS Vector b at time "<<currentTime<<"s ---\n";
        cout<<"[ ";
        for(double val:b) {
        ...
        // Corrected indexing for prev_branch_currents
        for(const auto& pair:voltageSourceNameToCurrentIndex) {
            // pair.second holds the absolute MNA matrix index for this branch current variable
        ...
        for(auto& el:elements) {
            ...
            el->updateTransientState(prev_voltages,prev_branch_currents,
                nodeToIndex,voltageSourceNameToCurrentIndex,actual_dt);
        }
