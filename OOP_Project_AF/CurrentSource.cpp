...
CurrentSource::CurrentSource(const string& name,const string& node1,const string& node2,const string& dcValueStr)
...
CurrentSource::CurrentSource(const string& name,const string& node1,const string& node2,
...
{
    if(isACSource())
        return "CurrentSource "+name+" "+node1+" "+node2+" AC Mag="+to_string(ac_magnitude)
        +"A Phase="+to_string(ac_phase)+"deg Freq="+std::to_string(ac_frequency)+"Hz";
    else
