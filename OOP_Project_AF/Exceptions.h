...
public:
    explicit CircuitError(const std::string& message)
        : std::runtime_error("Circuit Error: "+message) {}
};
...
public:
    explicit ElementNotFoundError(const std::string& name)
        : CircuitError("Element '"+name+"' not found in the circuit.") {
    }
};
...
public:
    explicit NodeNotFoundError(const std::string& name)
        : CircuitError("Node '"+name+"' not found in the circuit.") {
    }
};
...
public:
    explicit UnsupportedTypeError(const std::string& type)
        : CircuitError("Unsupported element type: '"+type+"'.") {
    }
};
...
public:
    explicit InvalidValueError(const std::string& message)
        : CircuitError("Invalid value: "+message) {
    }
};
...
public:
    explicit MatrixError(const std::string& message)
        : CircuitError("Matrix Error: "+message) {
    }
};
...
public:
    explicit SyntaxError(const std::string& message)
        : CircuitError("Syntax Error: "+message) {
    }
};
